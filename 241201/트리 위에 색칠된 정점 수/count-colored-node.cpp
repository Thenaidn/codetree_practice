...
int paintedafterthis[MAXN];


// DFS를 통해 깊이와 1차 부모 설정
void dfs(int node, int d, int cnt) {
    visited[node] = true;
    depth[node] = d;
    if(painted.find(node) != painted.end()){cnt++;}
    paintedafterthis[node] = cnt;


    for (int child : tree[node]) {
        if (!visited[child]) {
            parent[0][child] = node; // 1차 부모 설정
            dfs(child, d + 1, cnt);       // 자식으로 이동
        }
    }
}
...
int main() {
    ...
    while (q--) {
        ...
        cnt = paintedafterthis[u] - paintedafterthis[lca] 
        + paintedafterthis[v] - paintedafterthis[lca] ;
